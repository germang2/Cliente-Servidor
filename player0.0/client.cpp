...
void messageToFile(const message &msg, bool part) {
  const void *data;
  msg.get(&data, 2); // the first is the name "file", the second is the parts,
...
void songManager(Music *music, SafeQueue<string> &playList, bool &stop,
  ...
  s.connect("tcp://192.168.13.149:5555");
  message m, n;
  ...
  while (true) {
    ...
    while ((music->getStatus() == SoundSource::Playing and !stop) or
           music->getStatus() == SoundSource::Paused) {
      elapsed2 = clock.getElapsedTime(); // do not restart the clock
      if ((elapsed2 >= current_song_duration - delta) and
